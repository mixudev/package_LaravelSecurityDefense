...
@if(count($quarantinedIps) > 0)
    <section class="rounded-xl bg-white dark:bg-[#0f172a] border border-slate-200/90 dark:border-slate-800 p-4 sm:p-5 shadow-xs space-y-3.5">
        <div class="flex items-center justify-between">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <h3 class="text-sm font-bold text-slate-900 dark:text-white tracking-tight flex items-center space-x-2">
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Threat actors temporarily isolated from application network boundaries.</p>
                </div>
            </div>
        </div>
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-slate-800/80 bg-white dark:bg-[#0f172a]">
                    @foreach($quarantinedIps as $jail)
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition">
                            ...
                            <td class="px-4 py-3 text-slate-700 dark:text-slate-300">{{ $jail->reason }}</td>
                            <td class="px-4 py-3 text-slate-500 dark:text-slate-400 whitespace-nowrap">{{ $jail->jailed_at ? $jail->jailed_at->diffForHumans() : '—' }}</td>
                            <td class="px-4 py-3 text-slate-500 dark:text-slate-400 whitespace-nowrap">
                                @if($jail->expires_at)
                                    <span class="{{ $jail->expires_at->isPast() ? 'text-slate-400 dark:text-slate-500' : 'text-amber-600 dark:text-amber-400 font-semibold' }}">{{ $jail->expires_at->diffForHumans() }}</span>
                                @else
                                    —
                                @endif
                            </td>
                            <td class="px-4 py-3 text-right">
                                <form action="{{ route('security-defense.quarantine.pardon') }}" method="POST" class="inline">
                                    ...
                                    <button type="submit" aria-label="Release IP {{ $jail->ip }} from quarantine" title="Release {{ $jail->ip }}"
                                        class="inline-flex items-center px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-emerald-100 text-slate-700 hover:text-emerald-800 dark:bg-slate-800 dark:hover:bg-emerald-950 dark:text-slate-300 dark:hover:text-emerald-400 border border-slate-200 dark:border-slate-700 transition cursor-pointer text-xs font-semibold shadow-xs">
